...
def analyze_system_accounts():
    ...
    for user, homedir in users:
        ...
        if os.path.exists(history_file):
            try:
                ...
                # 提取最近的历史命令（最多50条）
                recent_commands = [line.strip() for line in history_lines if line.strip()][-50:]
                if recent_commands:
    ...
    # 输出用户历史操作命令
    print(f"\n{Colors.BOLD_YELLOW}=== 三天内用户历史操作命令 ==={Colors.RESET}")
    if account_results["user_history_commands"]:
        ...
        print(f"{Colors.GREEN}未检测到三天内用户历史操作命令{Colors.RESET}")

    # 输出新增与删除操作命令记录
    print(f"\n{Colors.BOLD_YELLOW}=== 三天内新增与删除操作命令记录 ==={Colors.RESET}")
    if account_results["add_delete_commands"]:
        ...
        print(f"{Colors.GREEN}未检测到三天内新增与删除操作命令记录{Colors.RESET}")
...
def generate_emergency_report(all_results):
    ...
    with open(txt_report_path, 'w', encoding='utf-8') as f:
        ...
        if account_results.get('user_history_commands', {}):
            ...
            f.write("未检测到三天内用户历史操作命令\n\n")
        
        # 三天内新增与删除操作命令记录
        if account_results.get('add_delete_commands', {}):
            f.write("三天内新增与删除操作命令记录:\n")
            for user, commands in account_results['add_delete_commands'].items():
