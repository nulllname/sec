...
def analyze_ssh_logs():
    ...
    sorted_ips = sorted(ssh_results['brute_force_ips'].items(), key=lambda x: x[1], reverse=True)
    if sorted_ips:
    ...
    # 显示详细的SSH爆破记录（包含时间）
    print(f"\n{Colors.BOLD_YELLOW}=== SSH爆破详细记录（所有）==={Colors.RESET}")
    if ssh_results['brute_force_details_5days']:
        ...
        print(f"{Colors.GREEN}未检测到SSH爆破详细记录{Colors.RESET}")

    print(f"\n{Colors.BOLD_YELLOW}=== SSH成功登录记录（按时间倒序）==={Colors.RESET}")
    if ssh_results['successful_logins']:
        for idx, login in enumerate(ssh_results['successful_logins'], 1):
...
def analyze_system_accounts():
    ...
    if account_results["login_history"]:
        for idx, login in enumerate(account_results["login_history"], 1):
